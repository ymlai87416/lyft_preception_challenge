...
class_weights = np.ones((256, 800, 13))
class_weights[:, :, 7] += 1
class_weights[:, :, 10] += 4
    ...
    def get_batches_fn(batch_size):
        ...
        while (True):
            for batch_i in range(0, len(image_paths), batch_size):
                ...
                # print("any nan? ", np.isnan(images).any(), np.isnan(gt_images).any())
...
def gen_batch_function_c(data_folder, image_shape):
    """
    Generate function to create batches of training data
    :param data_folder: Path to folder that contains all the datasets
    :param image_shape: Tuple - Shape of image
        ...
        while (True):
            for batch_i in range(0, len(image_paths), batch_size):
                ...
                for image_file in image_paths[batch_i:batch_i + batch_size]:
                    ...
                    gt_image_p = preprocess_labels(scipy.misc.imread(gt_image_file))
                    gt_image_p = crop_image(gt_image_p)
                    gt_image_p = scipy.misc.imresize(gt_image_p, image_shape)
...
def gen_test_output(sess, softmax, keep_prob, image_pl, data_folder, image_shape):
    ...
    for image_file in glob(os.path.join(data_folder, 'CameraRGB', '*.png')):
        ...
        im_softmax[0].reshape(image_shape[0], image_shape[1], num_classes)
        im_argmax = np.argsort(im_softmax, axis=2)[:, :, num_classes - 1].reshape(image_shape[0], image_shape[1])
        im_argmax = scipy.misc.imresize(im_argmax.astype(np.uint8), image_origin_shape, interp='nearest').reshape(
            image_origin_shape[0], image_origin_shape[1], 1)
...
def gen_test_output_2(sess, softmax, keep_prob, image_pl, data_folder, image_shape):
    ...
    for image_file in glob(os.path.join(data_folder, 'CameraRGB', '*.png')):
        ...
        image_origin_shape = (image_origin.shape[0], image_origin.shape[1])
        # image = scipy.misc.imresize(image_origin, image_shape)
        image = cv2.resize(image_origin, (image_shape[1], image_shape[0]), interpolation=cv2.INTER_CUBIC)
        ...
        im_argmax = cv2.resize(im_argmax.astype(np.uint8), (image_origin_shape[1], image_origin_shape[0]),
                               interpolation=cv2.INTER_NEAREST).reshape(image_origin_shape[0], image_origin_shape[1], 1)
...
def gen_test_output_3(sess, softmax, keep_prob, image_pl, data_folder, image_shape):
    """
    Generate test output using the test images
    :param sess: TF session
    :param logits: TF Tensor for the logits
    ...
    for image_file in glob(os.path.join(data_folder, 'CameraRGB', '*.png')):
        ...
        im_argmax = np.argmax(im_softmax[0], axis=2)
        # im_argmax = np.argsort(im_softmax, axis=2)[:, :, num_classes - 1].reshape(image_shape[0], image_shape[1])
        # im_argmax = cv2.resize(im_argmax[0].astype(np.uint8), (image_origin_shape[1], image_origin_shape[0]),
        #                       interpolation=cv2.INTER_NEAREST).reshape(image_origin_shape[0], image_origin_shape[1], 1)
        im_argmax = pad_image(im_argmax).reshape((600, 800, 1))

        im_blue = np.zeros((600, 800, 1), dtype=np.uint8)
        im_green = np.zeros((600, 800, 1), dtype=np.uint8)
...
def save_inference_samples_3(runs_dir, data_dir, sess, image_shape, softmax, keep_prob, input_image):
    # Make folder for current run
    output_dir = os.path.join(runs_dir, str(time.time()))
    if os.path.exists(output_dir):
        shutil.rmtree(output_dir)
    ...
    # Run NN on test images and save them to HD
    print('Saving test images to: {}'.format(output_dir))
    image_outputs = gen_test_output_3(
        sess, softmax, keep_prob, input_image, os.path.join(data_dir, 'Test'), image_shape)
    for name, image in image_outputs:
        scipy.misc.toimage(image, cmin=0, cmax=255).save(os.path.join(output_dir, name))
